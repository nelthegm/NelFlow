...
test("presentation cleanup preserves native records and guarded Undo path", () => {
  ...
  assert.match(chatUi, /\["player-strike", "multi-target-strike", "spell-attack"\]\.includes/);
});
...
test("0.14.14 metadata preserves GitHub distribution structure", () => {
  ...
  assert.equal(packageMetadata.version, "0.14.14");
  assert.equal(module.manifest, "https://raw.githubusercontent.com/nelthegm/NelFlow/main/module.json");
  assert.equal(module.download, "https://github.com/nelthegm/NelFlow/releases/download/v0.14.14/nelflow.zip");
});
