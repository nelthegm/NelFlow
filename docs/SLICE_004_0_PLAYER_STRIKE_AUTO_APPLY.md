# Slice 4.0 / Nelflow 0.6.3: Deterministic Character Strike Damage
...
The 30-second client-local intent records schema version 1, exact attack message and transaction IDs, nonce, requested native variant, clicking user, source actor/token/item, Strike identifier and action index, alternate usage, attack outcome, scene, and available combat/round/turn context. The authoring client's `preCreateChatMessage` hook accepts exactly one fresh structurally matching intent and adds `flags.nelflow.characterStrikeCorrelation` without touching PF2e flags or visible content. That operation changes the local hint from pending to bound; it does not finalize or consume the relationship. Cancelled dialogs leave an unbound hint that expires without changing durable transaction state.

The bound tuple is transaction ID, source attack-message ID, intent nonce, and damage-message ID. A repeated observation of that exact tuple is accepted idempotently. The same nonce with another damage message, or the same damage message with another transaction, is a conflict and never applies. Once bound, the 30-second pending timeout no longer applies: the elected GM uses the persisted message metadata and transaction link, so refresh, disconnect, socket delay, and ordinary processing latency do not erase correlation.

The elected GM treats the flag as an untrusted causal hint. It re-reads the exact source attack, transaction, native damage message, actor, item, owner permission, recorded target, setting, state, nonce shape, author, and variant. Age is validated only before binding. Correlation priority is: validated click intent; any future exact native linkage; structured PF2e fallback; Manual Review for an actual observed conflict. A direct link therefore selects Attack B even if Attack A is structurally identical and waiting.
...
Each attack transaction and damage message can claim each other once. The elected GM reserves that relationship in the damage-claim registry before persisting the bound transaction, then advances through damage-observed, validating, claimed, applying, and applied. Binding is causal identity; claiming is authority; application is mechanics; finalization occurs only after the native application succeeds. A stable elected active GM, per-attack queue, current transaction state, revisioned durable flag, and the claim registry prevent repeated hooks, socket wake-ups, and other GM clients from applying twice. A valid direct relationship short-circuits structured fallback, including when fallback has zero or multiple heuristic candidates. Without a direct intent, an actually observed message matching multiple transactions is deliberately ambiguous.

## Authority and socket security

The module socket accepts exactly `{ action: "player-strike-damage-observed", damageMessageId }`, with a bounded document-ID character set. Any extra target, actor, source-message, outcome, variant, formula, total, roll, HP delta, or other field invalidates the payload. The socket is only a wake-up: the elected GM re-reads the native damage message, canonical attack transaction, attack message, source actor/item, current ownership, setting, exact target token/actor, disposition, outcome, variant, and state. Author activity was required when the attack transaction was created; disconnect after a durable binding does not invalidate it. The normal cross-client `createChatMessage` hook is also safe because it enters the same queued claim path.
...
PF2e 8.4.0 exposes no conclusive structured field proving that a particular target currently has a legal Shield Block choice. Nelflow 0.6.3 therefore does not guess or implement a reaction gate. It never selects Shield Block, consumes a reaction, damages a shield, computes Hardness, or presents a reaction prompt. The existing adapter uses `shieldBlockRequest: false`; tables needing reaction decisions should leave this setting Off until the later reaction-gate slice.
...
### Nelflow 0.6.5 presentation projection

Nelflow 0.6.5 projects Waiting, Applying, Applied, Manual Review, Interrupted, Miss, Undone, or Undo Blocked once per logical transaction. The viewer-visible native damage message is preferred; the exact attack and application message IDs are deterministic fallbacks when earlier records are unavailable to that viewer. Only that host receives the concise application line and guarded Undo. The legacy Strike fallback and other linked cards suppress duplicate Nelflow-owned status and Undo.

Transaction internals are never inserted into ordinary chat. Failed, interrupted, ambiguous, orphaned, recovery, and failed-Undo states receive only concise GM recovery wording and **Review** on the same canonical host. The review dialog retains guarded recovery actions and sanitized support export without displaying correlation IDs, authority evidence, audit records, or raw flags. The legacy diagnostics setting remains registered and hidden for stored-value compatibility; no value can restore chat internals.
