...
Stage 1 — immediate attack check (new):

```text
nelflow.strikeAttackResolvedPresentation
```

Stage 2 — existing damage / final resolved feed:
...
Both stages share `transactionId = nelflow-${attackMessageId}`. PC misses emit
Stage 1 only. Hits emit Stage 1 on attack and Stage 2 after damage. Native PC
cards stay unchanged. `nelflow.strikeResolved` remains NelCine-only.

See [0.14.5 release notes](docs/RELEASE_NOTES_0.14.5.md) and
[runtime plan](docs/NELFLOW_0.14.5_TEST_PLAN.md).
